/*
  ...
  - You are about to drop the column `PaymentDuration` on the `rent` table. All the data in the column will be lost.
  - Added the required column `paymentDuration` to the `Rent` table without a default value. This is not possible if the table is not empty.
...
-- DropIndex
DROP INDEX `subscription_mallId_fkey` ON `subscription`;
...
-- AlterTable
ALTER TABLE `notification` MODIFY `type` ENUM('BID', 'REQUEST', 'MESSAGE', 'SYSTEM', 'PAYMENT', 'ALERT') NOT NULL;

-- AlterTable
ALTER TABLE `rent` DROP COLUMN `PaymentDuration`,
