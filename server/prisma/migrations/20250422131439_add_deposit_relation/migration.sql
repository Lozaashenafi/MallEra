/*
  ...
  - You are about to drop the column `bidId` on the `deposit` table. All the data in the column will be lost.
  - You are about to drop the column `discription` on the `rentinfo` table. All the data in the column will be lost.
  - A unique constraint covering the columns `[depositId]` on the table `Bid` will be added. If there are existing duplicate values, this will fail.
  - Added the required column `depositId` to the `Bid` table without a default value. This is not possible if the table is not empty.
...
-- AlterTable
ALTER TABLE `bid` ADD COLUMN `depositId` INTEGER NOT NULL;

-- AlterTable
ALTER TABLE `deposit` DROP COLUMN `bidId`;

-- AlterTable
ALTER TABLE `rentinfo` DROP COLUMN `discription`,
    ADD COLUMN `description` VARCHAR(191) NULL;

-- CreateIndex
CREATE UNIQUE INDEX `Bid_depositId_key` ON `Bid`(`depositId`);
